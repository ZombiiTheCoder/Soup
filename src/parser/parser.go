...
func Parse(text string){
	...
	parse_obj_expr := func() Expr {
		...
		for (notEof && at().Type != kind.ClosedBrace){
			...
			if (at().Type == kind.Comma){
				next()
				properties = append(properties, Propertykinds.Property, key})
				continue
			} else if (at().Type == kind.ClosedBrace) {
				properties = append(properties, Property{kinds.Property, key})
				continue
			}
	...
	parse_assign_expr := func() Expr {
		...
		if (at().Type == kind.Equals){
			...
			return AssignExpr{kinds.AssignExpr, left, val}
		}
	}
